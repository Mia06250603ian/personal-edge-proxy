...
混淆和上面那些不在同一个维度上：它改变的是流量**看起来像什么**，不影响端口、
不影响路径、不影响拥塞控制，**不是这次耦合的来源**。而且它已经在服务端和两台
客户端上都跑着，是当前少数几个两端一致的东西。

留着它还省掉一次两端同时改的机会——**混淆是两端字段，只改一边的结果是"连不上"，
而不是"变慢"**，最容易被当成新故障。这次回退已经要动不少地方，不必再多这一处。

`restore-baseline.sh` 默认保留混淆并沿用现有密码。真要拆的话加 `--no-obfs`，
但**必须同一次把两台客户端的 `obfs` 字段也删掉**。
...
| 混淆 | **Salamander，保留**（唯一被刻意留下的非基线项，见 §0） |
...
如果哪天要**拆掉**混淆：服务端 `bash /root/rb.sh --no-obfs`，
同时把两台客户端的 `obfs` / `obfs-password` 删掉，**同一次做完**。
少改一边就是节点恒失败，而且失败形式是"连不上"，很容易被误判成别的问题。

顺带记住它的定位：混淆改变的是流量**看起来像什么**，只覆盖 HY2。
它是最后一张牌——开着混淆手机仍然不通的话，就指向这个 IP 在该运营商蜂窝网上
被针对，再往端口和参数上绕没有意义。
