...
> 🔴 **原先这里写着「缓冲区那一项有可见效果：上传从个位数变成 48 Mbps」。那句话是错的，已删除。**
> 从来没有测过改动之前的上传速度，这个因果是凭空补的。当晚随后跑的 `nstat` 给出了真实数字：
>
> ```text
> UdpRcvbufErrors  33        开机 6 小时内因缓冲区满而丢弃的 UDP 包
> UdpInDatagrams   297435    同期收到的总数   →  0.011%
> ```
>
> 缓冲区确实溢出过，但**少到可以忽略**。`docs/AUDIT-RESULT.md` 二.2 三天前就写过
> 「上限低 ≠ 正在溢出，这是推测不是诊断结论」，并点名了这两项该查的计数器——
> 当晚照样没查就改了，还给它编了一个效果。**改动本身无害，可以留着，但它不是任何
> 问题的解。**
...
### 深夜补充：nstat 把「服务器有没有在丢包」钉死了

```text
UdpSndbufErrors  0         发送方向一个包都没丢
UdpRcvbufErrors  33        接收方向 33 个（占 0.011%，可忽略）
ping 1.1.1.1               0.96ms，0% 丢包
load average               0.00
```

**服务器在发送方向零丢包。** 所以 iPad 量到的 50% 丢包，**全部发生在包离开服务器
之后**——这次是计数器说的，不是排除法推的。

### 🔶 下一步该试的：把 Brutal 开回来（尚未验证）

用户说「刚搭上的时候还是 300 左右」。查记录：**那时 Brutal 是开着的**
（§3 基线：72 小时内 234/235 条连接为 `tx: 18750000`）。09-06 关掉了它。

而 Brutal 的设计目的**正是为丢包严重的链路准备的**——故意无视丢包、按声明速率硬推。
BBR 相反，遇到 50% 丢包会把发送速率压到地板上。

**所以在一条本来就丢一半包的路上，关掉 Brutal 很可能就是变慢的直接原因。**
...
|---|---|---|
| 丢包链路上的速度 | 快 | 慢 |
| 代价 | 持续高速 UDP，**§0.8 怀疑它正是招来运营商 QoS 的原因**——也就是手机断线那件事 | 温和，不招惹中间设备 |

当初关它是为了治手机断线；**代价可能就是现在的慢。** 两者不能兼得，得测。
...
然后 iPad 上测速对比。退回把 `false` 改回 `true` 即可。
⚠️ 一次只改这一样，否则和线路时段因素混在一起没法归因。
